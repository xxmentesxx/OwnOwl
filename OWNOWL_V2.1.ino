...
#include "sound_data.h" // HEX ses verileri burada!
#include <WiFi.h>
#include <ESPmDNS.h>
#include <WiFiUdp.h>
...
const int MOTOR_MAX_STEP_PER_LOOP = 30;  // Döngü başına max adım (~75ms blok, sonra yeni veri oku)
...
// =============================================================
//  KAMERA UART PARSER — Anlık Takip
// =============================================================
// Sıçrama/ardışıklık filtreleri KALDIRILDI — yön değişikliklerini
// engelliyordu. EMA filtresi zaten yumuşatma yapıyor.
// =============================================================

String camBuffer = "";
const int LOSE_FRAMES = 8;
int consecutiveN = 0;
...
void parseCameraData() {
        ...
        if (c == '\n') {
            ...
            if (camBuffer.length() >= 5 && 
                ...
                if (parsed == 4) {
                    ...
                    // Sadece boyut kontrolü
                    if (fw >= 15 && fh >= 15 && fw <= 300 && fh <= 300) {
                        faceX = fx;
                        faceY = fy;
                        faceW = fw;
                        faceH = fh;
    ...
    if (millis() - lastRxLog > 5000) {
        Serial.printf("[FILTER] ArdN:%d Yuz:%s\n", 
            consecutiveN,
...
void trackFace() {
    ...
    // --- MOTOR TAKİBİ ---
    // Kamera motora bağlı: motor döndükçe kamera da döner
    // Yüz merkeze gelene kadar dön, overshoot yapma
    int errorX = (int)smoothFaceX - (FRAME_W / 2);
    
    if (abs(errorX) > 50) {  // Geniş dead zone: merkeze yakınsa dönme
        // Orantılı kontrol: hata büyükse hızlı, küçükse yavaş
        int steps = -(errorX / 4);
        steps = constrain(steps, -80, 80);
        moveMotor(steps);
    }
}
...
// Bellekten (sound_data.h) WAV oynatıcı
// Header (44 byte) atlanır
void playMemoryWav(const unsigned char* wavData, size_t length) {
    if (length <= 44) return;
    
    // Header atla
    const unsigned char* p = wavData + 44;
    size_t remaining = length - 44;
    size_t chunk_size = 512;
    size_t bytes_written;

    while (remaining > 0) {
        size_t to_write = (remaining < chunk_size) ? remaining : chunk_size;
        i2s_write(I2S_NUM_0, p, to_write, &bytes_written, portMAX_DELAY);
        p += to_write;
        remaining -= to_write;
    }
...
void playHootSound() {
    // sound_data.h içindeki 'woho_wav' dizisi
    playMemoryWav(woho_wav, sizeof(woho_wav));
}

// Selamlama melodisi — yüz ilk tespit edildiğinde çalar
// Tatlı, yükselen tonlar (robot "merhaba!")
unsigned long lastGreetTime = 0;
const unsigned long GREET_COOLDOWN = 30000; // 30 saniye tekrarlama

void playGreeting() {
    if (millis() - lastGreetTime < GREET_COOLDOWN) return;
    lastGreetTime = millis();
    
    Serial.println("[SES] Hello!");
    Serial.println("[SES] Hello!");
    playMemoryWav(hello_wav, sizeof(hello_wav));
    
    // Selamlama kirpmasi
    blinkNow = true;
}

// Kızgın ses — hızlı inen-çıkan rahatsız edici tonlar
void playAngrySound() {
    // Alarm gibi: viii-vooo-viii-vooo! 🚨
    for(int i=0; i<3; i++) {
        playTonePacket(1200, 50, 20000);
        playTonePacket(800, 50, 20000);
    }
    i2s_zero_dma_buffer(I2S_NUM_0);
}
...
void loop() {
    ...
    if (!isAngry) {
        if (faceDetected) {
            trackFace();
            playGreeting(); // İlk yüz tespitinde selamla (cooldown içinde tekrarlamaz)
        } 
    ...
    // --- 6. RASTGELE BAYKUŞ SESİ 🦉 ---
    // --- 6. RASTGELE BAYKUŞ SESİ (woho) 🦉 ---
    static unsigned long nextHootTime = 0;
    
    // İlk 30 saniye: Sık çal (her ~7 saniyede bir)
    // Sonra: Seyrek çal (2-4 dakika arası)
    if (nextHootTime == 0) nextHootTime = millis() + 5000;

    if (millis() > nextHootTime && !isAngry) {
        Serial.println("[SES] Rastgele Baykus Sesi (WOHO) 🦉");
        playHootSound(); // woho.wav oynatır
        blinkNow = true; 
        
        if (millis() < 30000) {
            // İlk 30 saniye içindeyiz → sık çal
            nextHootTime = millis() + random(5000, 9000); 
        } else {
            // Normal mod → seyrek çal
            nextHootTime = millis() + random(120000, 240000); 
        }
    }
